...
class Actions:
    ...
    def export_all(cls, **kwargs):
        ...
        for matiere in Infos.data_path.keys():
            ...
            all_matiere_path += [("./html/"+matiere+".html", matiere)]

        with open(Infos.path+"/index.html", "w") as f:
            index_data = '<meta charset="UTF-8">'+dark_mode()
...
for i in cls.file_data.split("\n"):
    ...
    def create_html_file(cls, matiere, inter, cours):
        ...
        else:  # Verifier si y'a [^\\]$.+[^\\]$ ou same with $$ ou with \\(\\)
            try:
                math = re.search(r"(\$.+\$)|(\$\$.+\$\$)|(\\\\\(.+\\\\\))", open(Infos.path+"/notes/"+matiere+"/"+inter+"/"+cours, "r").read()) is not None
            except:
                math = None
        ...
        with open(html_inter_path+"/"+cours+".html", "w", encoding="utf-8") as f:
            f.write(multi_markdown(Infos.path+"/notes/"+matiere+"/"+inter+"/"+cours , math, True))
...
def multi_markdown(filename, math, dark):
    ...
    return model(result.stdout.decode('utf-8', 'replace'), math, dark)
